...
class Peminjamankendaraan_model extends CI_Model
{
    ...
    public function detailkendaraan($xbarang)
    {
        $query = "SELECT 
                    u.unit_pengguna,u.unit_pengguna as nama_unit, 
                    k.no_pol, 
                    (CONCAT('Kendaraan Dengan No Pol ',k.no_pol)) as nama_barang,k.file,
                    k.no_rangka,k.merk,k.no_mesin,jr.nama_jenis_ranmor,k.tahun_perolehan,k.asal_perolehan,jb.nama_jenis_bbm,ko.kondisi,k.tipe_ranmor
                    FROM kendaraan k
                    LEFT JOIN unit_pengguna u on u.id = k.unit_pengguna
                    LEFT JOIN kondisi ko on ko.id = k.kondisi_ranmor
                    LEFT JOIN jenis_bbm jb on jb.id = k.jenis_bbm
                    LEFT JOIN jenis_ranmor jr on jr.id = k.jenis_ranmor
                    WHERE k.id IN ($xbarang)";
        return $this->db->query($query)->result_array();
        // die($query);
        // var_dump($query);
        echo json_encode($query);
